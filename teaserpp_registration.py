...
class TeaserReg:
    """
    A specific implementation of registration using the Teaser++ model.
    """

    def __init__(self):
        self.paths = {}

    def run(self, iters=7):
        print("--- Registering Point Clouds with TeaserReg ---")

        # scene_dir_name = f"{self}_scene"
        # trace_output_file = os.path.join(
        #     self.paths["trace_output_dir"], scene_dir_name, "output.pt"
        # )
        base = "hovering_drone"
        trace_dir = os.path.join(
            "sample_outputs",
            base,
            "4_trace_anything_output",
            base + "_scene",
        )
        trace_output_file = os.path.join(trace_dir, "output.pt")

        self.paths["frames_scene_dir"] = os.path.join(trace_dir, "images")
        self.paths["masks_dir"] = os.path.join(trace_dir, "masks")
        self.paths["registration_output_dir"] = os.path.join(
            "sample_outputs", base, "6_registration"
        )

        # print(trace_output_file)
        ...
        frame_paths = sorted(
            [
                os.path.join(self.paths["frames_scene_dir"], f)
                for f in os.listdir(self.paths["frames_scene_dir"])
                if f.endswith(".png")
            ]
        )
        ...
        for iter in range(iters):
            for i in range(num_frames - 1):
                ...
                idx = np.random.choice(
                    min(len(pcd_t_full), len(pcd_t_plus_1_full)),
                    n_points_resample,
                    replace=False,
                )
                pcd_t_reg = pcd_t_full[idx].T
                pct_t_plus_1_reg = pcd_t_plus_1_full[idx].T

                solver.solve(pcd_t_reg, pct_t_plus_1_reg)
                ...
                transform_filename = os.path.join(
                    self.paths["registration_output_dir"],
            ...
            abs_error, normalized_error = self._save_error_summary(registration_errors)
            abs_error_hist.append(abs_error)
            normalized_error_hist.append(normalized_error)

        abs_error_hist = sorted(abs_error_hist)[1 : iters - 1]
        normalized_error_hist = sorted(normalized_error_hist)[1 : iters - 1]
        print(f"Trimmed mean over {iters - 2} runs:")
        print(f"Absolute error: {np.mean(abs_error_hist):.4f}")
        print(f"Normalized error: {np.mean(normalized_error_hist):.4f}")
    ...
    def _get_aligned_data_for_pair(self, trace_data, frame_paths, t, t_plus_1, device):
        ...
        for frame_idx in [t, t_plus_1]:
            mask_path = os.path.join(self.paths["masks_dir"], f"{frame_idx:03d}.png")
    ...
    def _save_error_summary(self, registration_errors):
        ...
        summary_path = os.path.join(
            self.paths["registration_output_dir"], "error_summary.json"
        )
        ...
        return error_summary.get("average_absolute_error", 0), error_summary.get(
