...
class PointCloudExtraction(PipelineComponent):
    ...
    def compute_voxel_size(self, pcd, alpha):
        """Compute voxel size for downsampling as a function of point density"""
        legacy_pcd = pcd.to_legacy()
        nn_dists = legacy_pcd.compute_nearest_neighbor_distance()
        self.median_dist = np.median(nn_dists)
        self.voxel_size = alpha * self.median_dist

    def pcd_postprocessing(
        self,
        object_points: torch.Tensor,
        n_points_resample=1024,
        compute_voxel_size=False,
    ) -> torch.Tensor:
        """Downsample point cloud and remove outliers"""

        points = o3c.Tensor.from_dlpack(torch.utils.dlpack.to_dlpack(object_points))
        pcd = o3d.t.geometry.PointCloud(points)

        # Use the same voxel spacing for each group of pointclouds
        if compute_voxel_size:
            self.compute_voxel_size(pcd, len(points) / n_points_resample)
        assert self.voxel_size is not None and self.median_dist is not None

        down_pcd = pcd.voxel_down_sample(self.voxel_size)

        # Filter out points that don't belong to largest cluster
        cluster_labels = down_pcd.cluster_dbscan(0.03, 10)
        unique, counts = np.unique(cluster_labels, return_counts=True)
        most_common_label = unique[np.argmax(counts)]
        cluster_points = down_pcd.point.positions[cluster_labels == most_common_label]
        cluster = o3d.t.geometry.PointCloud(cluster_points)

        # Final downsample to target number of points
        pcd = cluster.farthest_point_down_sample(n_points_resample)

        filtered_points = torch.utils.dlpack.from_dlpack(
    ...
    def run(self):
        ...
        if not os.path.exists(trace_output_file):
            raise FileNotFoundError(
                f"Trace Anything output file not found: {trace_output_file}."
            )
        if not os.path.exists(self.context.paths["masks_dir"]) or not os.listdir(
            self.context.paths["masks_dir"]
        ):
            raise FileNotFoundError(
                f"Masks directory not found or is empty: {self.context.paths['masks_dir']}."
            )
        ...
        for i in range(num_frames):
            ...
            if H0 > W0:
                ...
                original_frame_pil = original_frame_pil.transpose(
            ...
            object_points = self.pcd_postprocessing(
                object_points[:, 0, :], compute_voxel_size=i == 0
            )

            if object_points.shape[0] > 0:
                object_points_k0 = object_points
                pc_filename = os.path.join(
                    self.context.paths["pointclouds_dir"], f"pointcloud_{i:05d}.npy"
                )
